 ...
 *
 * <p>Try it. All three of these break payments, and they do not break it the same way:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>The first two hang, in every language, every time. A slow server accepts your
 ...
 * network partition looks like from the outside.
 *
 * <p>The third is the one that surprises people, because it is not one behaviour. A
 * stopped container loses its address and its DNS entry, and how long the failure takes
 * is decided by whichever gRPC library you happen to be using. Measured on this stack,
 * with no deadline anywhere: C# fails in about 4 seconds, Python, Go and Ruby in about
 * 20 (their own connect timeout), and Java — this one — was still waiting after 25.
 *
 * <p>Same outage, same contract, four seconds to never. <b>That</b> is the argument for
 * the deadline: without one, how long checkout hangs is a property of somebody else's
 * default rather than a number you chose — and 20 seconds is not "fast" for a checkout,
